...
class Logger:
    ...
    @staticmethod
    def load_cpk(checkpoint_path, inpainting_network=None, dense_motion_network=None, kp_detector=None, dt=None,
                 id_classifier=None,id_eraser=None,generation=None,
                 bg_predictor=None, avd_network=None, optimizer=None, optimizer_D=None, optimizer_bg_predictor=None,
                 optimizer_avd=None):
        ...
        if dt is not None:
            dt.load_state_dict(checkpoint['dt'])
        if id_classifier is not None:
            id_classifier.load_state_dict(checkpoint['id_classifier'])
        if id_eraser is not None:
            id_eraser.load_state_dict(checkpoint['id_eraser'])
        if generation is not None:
            generation.load_state_dict(checkpoint['generation'])
...
class Visualizer:
    ...
    def draw_image_with_kp(self, image, kp_array):
        ...
        for kp_ind, kp in enumerate(kp_array):
            rr, cc = circle(kp[1], kp[0], self.kp_size, self.kp_size, shape=image.shape[:2])
    ...
    def visualize(self, driving, source, out):
        ...
        if 'out' in out:
            prediction_post = out['out'].data.cpu().numpy()
            prediction_post = np.transpose(prediction_post, [0, 2, 3, 1])
            images.append(prediction_post)
