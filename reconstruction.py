...
def reconstruction(config, inpainting_network, dt, kp_detector, bg_predictor, dense_motion_network, checkpoint,
                   log_dir, dataset):
    png_dir = os.path.join(log_dir, 'reconstruction/png')
    log_dir = os.path.join(log_dir, 'reconstruction')

    if checkpoint is not None:
        Logger.load_cpk(checkpoint, inpainting_network=inpainting_network, kp_detector=kp_detector, dt=dt,
        ...
        with torch.no_grad():
            ...
            for frame_idx in range(x['video'].shape[2]):
                ...
                dense_motion = dense_motion_network(source_image=source, kp_driving=kp_driving,
                                                    ...
                                                    dropout_flag=False)
                w_id, w_m = dt(source, driving)
                out = inpainting_network(source, dense_motion, w_m)
