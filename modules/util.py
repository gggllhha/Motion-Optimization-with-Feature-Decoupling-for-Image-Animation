...
class Encoder(nn.Module):
    ...
    def __init__(self, block_expansion, in_features, num_blocks=3, max_features=512):
...
class IdentityEraser(nn.Module):
    def __init__(self, input_dim=512, hidden_dims=(256, 128), num_identities=500):
...
class IdentityClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dims, num_identities):
...
class MotionAttentionBlock(nn.Module):
    ...
    def __init__(self, in_dim, activation):
        super(MotionAttentionBlock, self).__init__()
        self.chanel_in = in_dim
        self.activation = activation

        self.query_conv = nn.Linear(in_dim, in_dim // 8)
    ...
    def forward(self, w_m, x):
        m_batchsize, C, width, height = x.size()
        proj_query = self.activation(self.query_conv(w_m)).view(m_batchsize, -1, 1, 1)  # bs 64 1 1
        proj_query = proj_query.expand(-1, -1, height, width).reshape(m_batchsize, -1, height * width).transpose(1,
                                                                                                                 2)  # bs 1024 64
        proj_key = self.activation(self.key_conv(x)).view(m_batchsize, -1, width * height)  # B X C x (W*H)
        energy = torch.bmm(proj_query, proj_key)  # transpose check
        attention = self.softmax(energy)  # BX (N) X (N)
        proj_value = self.activation(self.value_conv(x)).view(m_batchsize, -1, width * height)  # B X C X N
...
class IdentityClassifier_t(nn.Module):
    def __init__(self, input_dim, hidden_dims, num_identities):
        super().__init__()
        self.fc1 = nn.Linear(input_dim, hidden_dims[0])
        self.fc2 = nn.Linear(hidden_dims[0], hidden_dims[1])
...
if __name__ == '__main__':
    # 随机输入 + 随机标签
    t = IdentityClassifier_t(input_dim=512, hidden_dims=[1024, 1024], num_identities=500)
    opt = torch.optim.AdamW(t.parameters(), lr=2.0e-4)
    t.train()
    for _ in range(1000):
        inp = torch.randn(8, 512)
        lbl = torch.randint(0, 500, (8,))
        print(lbl)
        loss = F.cross_entropy(t(inp) / 0.1, lbl)
        loss.backward()
        opt.step()
        opt.zero_grad()
        print(loss.item())
