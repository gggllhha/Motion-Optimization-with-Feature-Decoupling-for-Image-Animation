...
from modules.util import AntiAliasInterpolation2d, TPS, IdentityEraser, IdentityClassifier
...
class GeneratorFullModel(torch.nn.Module):
    ...
    def __init__(self, kp_extractor, dt, id_eraser, generation, bg_predictor, dense_motion_network, inpainting_network,
                 train_params, dataset_params, *kwargs):
        ...
        self.id_eraser = id_eraser
        for param in self.id_eraser.parameters():
            param.requires_grad = False
    ...
    def forward(self, x, epoch):
        ...
        w_id, w_m = self.dt(x['source'], x['driving'])  # bs 512
        # out = self.generation(w_id=w_id, w_m=0)
        ...
        generated = self.inpainting_network(x['source'], dense_motion, w_id=None, w_m=w_m)
        generated.update({'kp_source': kp_source, 'kp_driving': kp_driving})
        generated.update({'w_m': w_m})
        # generated.update({'out': out})
        loss_values = {}

        pyramide_real = self.pyramid(x['driving'])
        pyramide_generated = self.pyramid(generated['prediction'])
        # pyramide_out = self.pyramid(out)
        ...
        #     value_total = 0
        #     for scale in self.scales:
        #         x_vgg = self.vgg(pyramide_out['prediction_' + str(scale)])
        #         y_vgg = self.vgg(pyramide_real['prediction_' + str(scale)])
        #
        #         for i, weight in enumerate(self.loss_weights['perceptual']):
        #             value = torch.abs(x_vgg[i] - y_vgg[i].detach()).mean()
        #             value_total += self.loss_weights['perceptual'][i] * value
        #     loss_values['perceptual_out'] = value_total
        ...
        # warp loss 2
        if self.loss_weights['warp_loss2'] != 0:
            occlusion_map = generated['occlusion_map']
            encode_map = self.inpainting_network.get_encode(x['driving'], occlusion_map)
            decode_map = generated['warped_encoder_maps_init']
            value = 0
            for i in range(len(encode_map)):
                value += torch.abs(encode_map[i] - decode_map[-i - 1]).mean()

            loss_values['warp_loss2'] = self.loss_weights['warp_loss2'] * value

        # bg loss
        if self.bg_predictor and self.loss_weights['bg'] != 0:
        ...
        # D loss
        # pred_m = self.id_eraser(w_m)
        # value = F.cross_entropy(pred_m, x['label'])
        # loss_values['LD'] = self.loss_weights['LD'] * value

        pred_m = self.id_eraser(w_m)
        ld_gen = F.cross_entropy(pred_m, x['label'])
        loss_values['Ladv'] = self.loss_weights['Ladv'] * ld_gen

        # R loss
        w_id_gt, w_m_gt = self.dt(generated['prediction'].detach(), generated['prediction'].detach())
        value = torch.abs(w_m - w_m_gt).mean()
        loss_values['LR'] = self.loss_weights['LR'] * value
