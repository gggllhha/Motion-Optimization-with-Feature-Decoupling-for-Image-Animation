...
from modules.util import ResBlock2d, SameBlock2d, UpBlock2d, DownBlock2d, MotionAttentionBlock
...
class InpaintingNetwork(nn.Module):
    ...
    def __init__(self, num_channels, block_expansion, max_features, num_down_blocks, multi_mask = True, **kwargs):
        ...
        self.MotionAttnBlock = MotionAttentionBlock(in_dim=512, activation=nn.ReLU())
    ...
    def forward(self, source_image, dense_motion, w_id=None, w_m=None):
        ...
        out_ij = self.deform_input(out.detach(), deformation[0])  # bs 512 32 32
        out = self.deform_input(out, deformation[0])
        out_ij_init = self.deform_input(out.detach(), deformation_init)

        out_ij = self.occlude_input(out_ij, occlusion_map[0].detach())  # bs 512 32 32
        ...
        if w_m is not None:
            out, _ = self.MotionAttnBlock(w_m, out)

        for i in range(self.num_down_blocks):
            out = self.resblock[2 * i](out)  # 512 32 32  512 64 64   256 128 128
            out = self.resblock[2 * i + 1](out)
            out = self.up_blocks[i](out)  # 256 64 64  128 128 128  64 256 256
            flow_index = i + 1  # 0 1 2
            encode_i = encoder_map[-(i + 2)]  # 256 64 64  128 128 128  64 256 256
        ...
        out = out * (1 - occlusion_last) + encode_i  # 64 256 256
        out = self.final(out)  # 3 256 256
