...
def train(config, inpainting_network, dt, id_eraser, generation, kp_detector, bg_predictor, dense_motion_network, checkpoint,
          log_dir,
          dataset):
    ...
    optimizer = torch.optim.Adam(
        [{'params': list(inpainting_network.parameters()) +
                    ...
                    list(dt.parameters()) +
                    list(generation.parameters()), 'initial_lr': train_params['lr_generator']}],
        lr=train_params['lr_generator'], betas=(0.5, 0.999), weight_decay=1e-4)
    optimizer_D = torch.optim.Adam(id_eraser.parameters(), lr=train_params['lr_generator'], betas=(0.5, 0.999), weight_decay=1e-4)
    ...
    if checkpoint is not None:
        start_epoch = Logger.load_cpk(
            checkpoint, inpainting_network=inpainting_network, dense_motion_network=dense_motion_network, dt=dt
            ,id_eraser=id_eraser,generation=generation,
            kp_detector=kp_detector, bg_predictor=bg_predictor,
            optimizer=optimizer, optimizer_D=optimizer_D, optimizer_bg_predictor=optimizer_bg_predictor)
    ...
    scheduler_optimizer_D = MultiStepLR(optimizer_D, train_params['epoch_milestones'], gamma=0.1,
                                        last_epoch=start_epoch - 1)
    if bg_predictor:
    ...
    generator_full = GeneratorFullModel(kp_detector, dt, id_eraser, generation, bg_predictor, dense_motion_network,
                                        inpainting_network, train_params, dataset_params)
    ...
    with Logger(log_dir=log_dir, visualizer_params=config['visualizer_params'],
        ...
        for epoch in trange(start_epoch, train_params['num_epochs']):
            for x in dataloader:
                ...
                clip_grad_norm_(dt.parameters(), max_norm=10, norm_type=math.inf)
                clip_grad_norm_(generation.parameters(), max_norm=10, norm_type=math.inf)
                ...
                for p in id_eraser.parameters():
                    p.requires_grad = True
                for _ in range(50):
                    w_m = generated['w_m'].detach()
                    pred_d = id_eraser(w_m)
                    d_loss = F.cross_entropy(pred_d, x['label']) * 10
                    d_loss.backward()
                    clip_grad_norm_(id_eraser.parameters(), max_norm=10, norm_type=math.inf)
                    optimizer_D.step()
                    optimizer_D.zero_grad()

                losses_generator.update({'LD': d_loss})
                losses = {key: value.mean().detach().data.cpu().numpy() for key, value in losses_generator.items()}
                logger.log_iter(losses=losses)
            scheduler_optimizer_D.step()
