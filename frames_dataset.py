...
def read_video(name, frame_shape):
    ...
    #         [img_as_float32(
    #             resize_fn(io.imread(os.path.join(name, frames[idx])))
...
class FramesDataset(Dataset):
    ...
    def __init__(self, root_dir, frame_shape=(256, 256, 3), id_sampling=False, is_train=True,
                 random_seed=0, pairs_list=None, augmentation_params=None, **kwargs):
        ...
        if os.path.exists(os.path.join(root_dir, 'train')):
            ...
            if id_sampling:
                train_ids = {os.path.basename(video).split('#')[0] for video in
                             os.listdir(os.path.join(root_dir, 'train'))}
                train_videos = list(train_ids)
            else:
        ...
        base_names = [os.path.splitext(v)[0] for v in self.videos]
        unique_ids = sorted(set(base_names))
        self.name2id = {name: idx for idx, name in enumerate(unique_ids)}
    ...
    def __getitem__(self, idx):
        ...
        identity_label = self.name2id.get(base_name, 0)
